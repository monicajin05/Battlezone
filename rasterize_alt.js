...
document.getElementById("myWebGLCanvas").style.background =
  'url("stars.jpg") center center / cover no-repeat';

// obj files
const OBJ_BODY_URL   = "Spaceshipbody.obj";  
const OBJ_FRONT_URL = "Spaceshipfront.obj";
const OBJ_BACK_URL = "Spaceshipback.obj"
...
function spawnObstacles() {
  ...
  for (let i = 0; i < NUM_ASTEROIDS; i++) {
    ...
    const y = rand(-FIELD_SIZE/4, FIELD_SIZE/4);   // float in space
    const z = rand(-FIELD_SIZE/2, FIELD_SIZE/2);

    const rx = 1.0 + Math.random() * 2.0;
    const ry = 1.0 + Math.random() * 2.0;
    const rz = 1.0 + Math.random() * 2.0;
...
function collision(testPos) {
  ...
  for (const ob of obstacles) {
    const ox = ob.pos[0], oy = ob.pos[1], oz = ob.pos[2];
    const rx = ob.radii[0], ry = ob.radii[1], rz = ob.radii[2];
...
function handleKeyDown(e) {
  ...
  const right = vec3.fromValues(Math.cos(tankHeading), 0, -Math.sin(tankHeading));
  ...
  switch (e.code) {
    case "KeyW": {
      vec3.scaleAndAdd(move, move, forward, MOVE_SPEED);
      didMove = true;
      break;
    }
  ...
  if (didMove) {
    const newPos = vec3.clone(tankPos);
    vec3.add(newPos, newPos, move);

    if (!collision(newPos) && !collidesWithEnemies(newPos)) {
      vec3.copy(tankPos, newPos);
    }
  }
}

// drawing spaceships
function drawSpaceship(position, heading, bodyColor, frontColor, backColor) {
  if (!meshes[0] || !meshes[1] || !meshes[2]) return;
  const scale = 2.0;

  const base = mat4.create();
  mat4.translate(base, base, position);
  mat4.rotateY(base, base, heading);
  mat4.scale(base, base, vec3.fromValues(scale, scale, scale));

  // Body
  const modelBody = mat4.clone(base);
  ...
  // Front
  const modelFront = mat4.clone(base);
  gl.uniformMatrix4fv(uModelLoc, false, modelFront);
  gl.uniform3fv(uColorLoc, frontColor);
  ...
  // Back
  const modelBack = mat4.clone(base);
  gl.uniformMatrix4fv(uModelLoc, false, modelBack);
  gl.uniform3fv(uColorLoc, backColor);
  gl.bindBuffer(gl.ARRAY_BUFFER, meshes[2].vbo);
  gl.vertexAttribPointer(aPosLoc, 3, gl.FLOAT, false, 0, 0);
  gl.bindBuffer(gl.ARRAY_BUFFER, meshes[2].nbo);
  gl.vertexAttribPointer(aNormLoc, 3, gl.FLOAT, false, 0, 0);
  gl.drawArrays(gl.TRIANGLES, 0, meshes[2].vertexCount);
}
...
function drawRadar() {
  ...
  gl.uniform3fv(uColorLoc, [0.05, 0.05, 0.10]);  

  // Use identity matrices (no projection)
  const identity = mat4.create();
  gl.uniformMatrix4fv(uModelLoc, false, identity);
  gl.uniformMatrix4fv(uViewLoc,  false, identity);
  gl.uniformMatrix4fv(uProjLoc,  false, identity);
  ...
  if (pyramidVBO && pyramidNBO) {
    ...
    for (const ob of obstacles) {
      ...
      mat4.scale(model, model, vec3.fromValues(ob.radii[0], ob.radii[1], ob.radii[2]));
  ...
  for (const e of enemies) {
    ...
    drawSpaceship(pos, e.heading, enemyBodyColor, enemyBodyColor, enemyBodyColor);
  }
...
async function main() {
  ...
  // load body, front, and back
  const bodyMesh   = await loadOBJ(OBJ_BODY_URL);
  const frontMesh = await loadOBJ(OBJ_FRONT_URL);
  const backMesh  = await loadOBJ(OBJ_BACK_URL);

  createMeshBuffers(bodyMesh,  [0.6, 0.8, 1.0]); // body 
  createMeshBuffers(frontMesh, [1.0, 0.9, 0.4]); // cockpit 
  createMeshBuffers(backMesh,  [0.9, 0.4, 0.4]); // engines
 
  meshes.push(bodyMesh, frontMesh, backMesh);
